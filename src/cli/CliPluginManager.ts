...
import {
  ...
  PluginSchemaItem,
} from "../plugins/TranscriptionPlugin";
...
export class CliPluginManager {
  ...
  getPluginInfo(name: string): PluginInfo | undefined {
    ...
    const modelSchema = schema.find(
      (s) => s.key === "model" && s.type === "model-select",
    );
  ...
  async downloadModel(
    ...
    if (!info?.requiresModelDownload) {
      throw new Error(
        `Plugin '${pluginName}' does not require model downloads`,
      );
    }
    ...
    if (!validModels.includes(modelName)) {
      throw new Error(
        `Invalid model '${modelName}' for plugin '${pluginName}'. Available: ${validModels.join(", ")}`,
      );
    }
  ...
  async validateApiKey(
    ...
    if (typeof plugin.validateApiKey !== "function") {
      return {
        valid: false,
        error: `Plugin '${pluginName}' does not support API key validation`,
      };
    }
