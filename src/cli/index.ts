...
program
  ...
  .description(
    "CLI for Whisper Mac transcription - supports multiple transcription engines",
  )
  .version("1.0.0");
...
program
  ...
  .action((pluginName, options) => {
    ...
    if (options.json) {
      ...
      originalConsole.log(
        `Type: ${info.requiresApiKey ? "Cloud API" : "Local"}`,
      );
      originalConsole.log(`Realtime: ${info.supportsRealtime ? "Yes" : "No"}`);
      originalConsole.log(
        `Batch Processing: ${info.supportsBatchProcessing ? "Yes" : "No"}`,
      );

      if (info.requiresApiKey) {
        originalConsole.log(`\nRequires API key: Yes`);
        originalConsole.log(
          `Set with: whisper-mac-cli set-api-key ${pluginName} <key>`,
        );
      }
...
program
  ...
  .action((pluginName, options) => {
    ...
    if (!info.requiresModelDownload) {
      originalConsole.log(
        `Plugin '${pluginName}' uses cloud-based models (no downloads required)`,
      );
      if (info.availableModels.length > 0) {
        originalConsole.log("\nAvailable models:");
        for (const model of info.availableModels) {
...
program
  ...
  .action(async (pluginName, modelName, options) => {
    ...
    originalConsole.log(
      `Downloading model '${modelName}' for ${info.displayName}...`,
    );

    try {
      const success = await pluginManager.downloadModel(
        pluginName,
        modelName,
        (msg, percent) => {
...
program
  ...
  .action(async (pluginName, apiKey, options) => {
    ...
    if (!info.requiresApiKey) {
      originalConsole.error(
        `Plugin '${pluginName}' does not require an API key`,
      );
      process.exit(1);
    }
...
program
  ...
  .option(
    "-k, --api-key <key>",
    "API key (for cloud plugins, overrides stored key)",
  )
  ...
  .action(async (file, options) => {
    ...
    if (!plugin) {
      originalConsole.error(
        `Plugin '${pluginName}' not found. Use 'list-plugins' to see available plugins.`,
      );
      process.exit(1);
    }
    ...
    const initialized = await pluginManager.initializePlugin(
      pluginName,
      pluginOptions,
    );
    if (!initialized) {
      if (info?.requiresApiKey && !options.apiKey) {
        originalConsole.error(
          `Plugin ${pluginName} requires an API key. Use --api-key or 'set-api-key' command.`,
        );
      } else {
