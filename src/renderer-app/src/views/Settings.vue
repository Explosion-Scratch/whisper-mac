...
<div class="window settings-root" v-cloak>
  <header class="toolbar toolbar-header">
    ...
    <div class="toolbar-actions">
      <button @click="resetAll" class="btn btn-default btn-icn-only action-btn"
        title="Reset all settings to defaults">
        <i class="ph ph-arrow-clockwise"></i>
      </button>
      <button @click="importSettings" class="btn btn-default btn-icn-only action-btn"
        title="Import settings from file">
        <i class="ph ph-download-simple"></i>
      </button>
      <button @click="exportSettings" class="btn btn-default btn-icn-only action-btn" title="Export settings to file">
  ...
  <div class="window-content" v-if="schema.length > 0">
    <nav class="sidebar">
      <div class="nav-group">
        <a v-for="section in schema" :key="section.id" class="nav-group-item"
          :class="{ active: currentSectionId === section.id }" @click="showSection(section.id)">
          <i class="icn ph" :class="getIcon(section.icon || section.id)"></i>
          {{ section.title }}
        </a>
        <a class="nav-group-item" :class="{ active: currentSectionId === 'about' }" @click="showSection('about')">
      ...
      <form @submit.prevent="saveSettings">
        <div v-if="currentSectionId !== 'permissions'" class="settings-section">
          <div class="section-header">
            <i class="ph-duotone" :class="getIcon(currentSection.icon || currentSection.id)"></i>
            <div>
            ...
            <div v-if="field.type === 'actions-editor' && settings.actions">
              <div class="actions-editor-container">
                ...
                <div class="actions-list">
                  <div v-if="!settings.actions?.actions?.length" class="empty-actions-state">
                  ...
                  <div v-for="(action, index) in settings.actions?.actions || []" :key="action.id"
                    class="action-card">
                    <div class="action-card-header" @click.self="toggleActionCard(action.id)">
                      <div class="action-info" @click="toggleActionCard(action.id)">
                        <div class="action-toggle">
                          <input type="checkbox" class="action-enabled-checkbox checkbox" v-model="action.enabled"
                            @click.stop />
                      ...
                      <div class="action-controls">
                        <button type="button" class="btn btn-icn-only btn-compact" @click="moveAction(index, -1)"
                          :disabled="index === 0" title="Move up">
                          <i class="ph ph-caret-up"></i>
                        </button>
                        <button type="button" class="btn btn-icn-only btn-compact" @click="moveAction(index, 1)"
                          :disabled="index === (settings.actions?.actions?.length || 0) - 1" title="Move down">
                          <i class="ph ph-caret-down"></i>
                        </button>
                        <button type="button" class="btn btn-icn-only btn-compact btn-negative"
                          @click="deleteAction(index)" title="Delete action">
                    ...
                    <div class="action-card-body" :class="{ collapsed: !expandedActions[action.id] }">
                      ...
                      <div class="config-section">
                        <div class="config-section-header" @click="toggleConfigSection(action.id, 'options')">
                          ...
                          <i class="ph ph-caret-down collapse-icon"
                            :class="{ collapsed: !isConfigSectionExpanded(action.id, 'options') }"></i>
                        </div>
                      ...
                      <div class="patterns-pills-container">
                        <template v-for="(pattern, pIndex) in action.matchPatterns || []"
                          :key="`pattern-${index}-${pIndex}`">
                          <div v-if="pattern" class="pattern-pill"
                            ...
                            <button type="button" class="pill-delete" @click.stop="deletePattern(index, pIndex)"
                        ...
                        <div
                          v-if="editingPattern && editingPattern.actionIndex === index && editingPattern.patternIndex !== undefined && settings.actions?.actions?.[index]?.matchPatterns?.[editingPattern.patternIndex]"
                          class="inline-edit-form pattern-edit-form">
                          <select class="form-control form-control-sm"
                            v-model="settings.actions.actions[index].matchPatterns[editingPattern.patternIndex].type">
                            <option value="startsWith">Starts with</option>
                            <option value="endsWith">Ends with</option>
                            <option value="exact">Exact</option>
                            <option value="regex">Regex</option>
                          </select>
                          <input type="text" class="form-control form-control-sm" placeholder="Pattern text..."
                            v-model="settings.actions.actions[index].matchPatterns[editingPattern.patternIndex].pattern"
                            @keydown.enter="closePatternEdit" @keydown.esc="closePatternEdit" />
                          <label class="checkbox-label-inline">
                            <input type="checkbox"
                              v-model="settings.actions.actions[index].matchPatterns[editingPattern.patternIndex].caseSensitive" />
                            Aa
                          </label>
                      ...
                      <div class="handlers-pills-container">
                        <template v-for="(handler, hIndex) in action.handlers || []"
                          :key="`handler-${index}-${hIndex}`">
                          <div v-if="handler" class="handler-pill-wrapper">
                            <div class="handler-pill" :class="{ expanded: isHandlerExpanded(index, hIndex) }"
                              ...
                              <button type="button" class="pill-delete" @click.stop="deleteHandler(index, hIndex)"
                                title="Delete handler">
                                <i class="ph ph-x"></i>
                              </button>
                            </div>
                            <div class="handler-config-panel"
                              v-show="isHandlerExpanded(index, hIndex) && handler.type">
                              <div class="handler-type-selector">
                                <label>Handler Type</label>
                                <select class="form-control" v-model="handler.type"
                              ...
                              <template v-if="handler.type === 'openUrl' && handler.config">
                                <div class="config-section">
                                  <div class="config-section-header"
                                    @click="toggleConfigSection(handler.id, 'basic')">
                                    <i class="ph ph-gear"></i>
                                    <span>Basic</span>
                                    <i class="ph ph-caret-down collapse-icon"
                                      :class="{ collapsed: !isConfigSectionExpanded(handler.id, 'basic') }"></i>
                                  </div>
                                  <div class="config-section-body"
                                    v-show="isConfigSectionExpanded(handler.id, 'basic')">
                                ...
                                <div class="config-section">
                                  <div class="config-section-header"
                                    @click="toggleConfigSection(handler.id, 'options')">
                                    <i class="ph ph-sliders"></i>
                                    <span>Options</span>
                                    <i class="ph ph-caret-down collapse-icon"
                                      :class="{ collapsed: !isConfigSectionExpanded(handler.id, 'options') }"></i>
                                  </div>
                                  <div class="config-section-body"
                                    v-show="isConfigSectionExpanded(handler.id, 'options')">
                                    <div class="config-field">
                                      <label class="checkbox-label">
                                        <input type="checkbox" v-model="handler.config.openInBackground" />
                                        Open in background
                                      </label>
                                    </div>
                                  </div>
                              ...
                              <template v-if="handler.type === 'openApplication' && handler.config">
                                ...
                                <div class="config-section">
                                  ...
                                  <div class="config-section-body"
                                    ...
                                    <div class="config-field">
                                      <label class="checkbox-label">
                                        <input type="checkbox" v-model="handler.config.waitForExit" />
                                        Wait for exit
                                      </label>
                                    </div>
                                  </div>
                              ...
                              <template v-if="handler.type === 'quitApplication' && handler.config">
                                ...
                                <div class="config-section">
                                  ...
                                  <div class="config-section-body"
                                    ...
                                    <div class="config-field">
                                      <label class="checkbox-label">
                                        <input type="checkbox" v-model="handler.config.forceQuit" />
                                        Force quit
                                      </label>
                                    </div>
                                  </div>
                                  ...
                                  <div class="config-section-body"
                                    ...
                                    <div class="config-field">
                                      ...
                                      <textarea class="form-control" rows="3"
                                        v-model="handler.config.command"></textarea>
                                    </div>
                                  </div>
                                </div>
                                <div class="config-section">
                                  ...
                                  <div class="config-section-body"
                                    ...
                                    <div class="config-field">
                                      <label class="checkbox-label">
                                        <input type="checkbox" v-model="handler.config.runInBackground" />
                                        Run in background
                                      </label>
                                    </div>
                                  </div>
                              ...
                              <template v-if="handler.type === 'segmentAction' && handler.config">
                                  ...
                                  <div class="config-section-body"
                                    ...
                                    <div class="config-field">
                                      ...
                                      <select class="form-control" v-model="handler.config.action">
                                        ...
                                        <option value="capitalizeFirstWord">Capitalize First Word</option>
                                ...
                                <div class="config-section"
                                  v-if="handler.config.action === 'replace' || handler.config.action === 'deleteLastN' || handler.config.action === 'removePattern'">
                                  <div class="config-section-header"
                                    @click="toggleConfigSection(handler.id, 'options')">
                                    <i class="ph ph-sliders"></i>
                                    <span>Options</span>
                                  ...
                                  <div class="config-section-body"
                                    v-show="isConfigSectionExpanded(handler.id, 'options')">
                                    <div v-if="handler.config.action === 'replace'" class="config-field">
                                      <label>Replacement Text</label>
                                      <input type="text" class="form-control" placeholder="{argument}"
                                        v-model="handler.config.replacementText" />
                                    </div>
                                    <div v-if="handler.config.action === 'deleteLastN'" class="config-field">
                                      <label>Count</label>
                                      <input type="number" class="form-control" min="1"
                                        v-model.number="handler.config.count" />
                                    </div>
                                    <div v-if="handler.config.action === 'removePattern'" class="config-field">
                              ...
                              <template v-if="handler.type === 'transformText' && handler.config">
                                  ...
                                  <div class="config-section-body"
                                    ...
                                    <div class="config-field">
                                      ...
                                      <input type="text" class="form-control"
                                        ...
                                        v-model="handler.config.replacePattern" />
                                    </div>
                                    <div class="config-field">
                                      <label>Replace Flags</label>
                                      <input type="text" class="form-control" placeholder="gimuy" maxlength="6"
                                        v-model="handler.config.replaceFlags" />
                                    </div>
                                    <div class="config-field"
                                      v-if="handler.config.replacementMode === 'literal' || !handler.config.replacementMode">
                                      <label>Replacement Text</label>
                                      <input type="text" class="form-control"
                                ...
                                <div class="config-section">
                                  <div class="config-section-header"
                                    @click="toggleConfigSection(handler.id, 'match')">
                                    <i class="ph ph-magnifying-glass"></i>
                                    <span>Match Conditions</span>
                                    <i class="ph ph-caret-down collapse-icon"
                                      :class="{ collapsed: !isConfigSectionExpanded(handler.id, 'match') }"></i>
                                  </div>
                                  <div class="config-section-body"
                                    v-show="isConfigSectionExpanded(handler.id, 'match')">
                                    <div class="config-field">
                                      <label>Match Pattern (Optional)</label>
                                      <input type="text" class="form-control"
                                        placeholder="Regex to match text (e.g. ^.{0,50}$)"
                                        v-model="handler.config.matchPattern" />
                                      <span class="field-help">Leave blank to apply to all text</span>
                                    </div>
                                    <div class="action-form-row">
                                      <div class="action-form-col">
                                      ...
                                      <div class="action-form-col">
                                        ...
                                        <input type="number" class="form-control" placeholder="Optional"
                                          v-model.number="handler.config.maxLength" />
                                      </div>
                                      <div class="action-form-col">
                                        <label>Min Length</label>
                                        <input type="number" class="form-control" placeholder="Optional"
                                          v-model.number="handler.config.minLength" />
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div class="config-section">
                                  <div class="config-section-header"
                                    @click="toggleConfigSection(handler.id, 'advanced')">
                                    <i class="ph ph-faders"></i>
                                    <span>Advanced</span>
                                    <i class="ph ph-caret-down collapse-icon"
                                      :class="{ collapsed: !isConfigSectionExpanded(handler.id, 'advanced') }"></i>
                                  </div>
                                  <div class="config-section-body"
                                    v-show="isConfigSectionExpanded(handler.id, 'advanced')">
                                    <div class="config-field">
                              ...
                              <div class="config-section">
                                <div class="config-section-header" @click="toggleConfigSection(handler.id, 'timing')">
                                  ...
                                  <i class="ph ph-caret-down collapse-icon"
                                    :class="{ collapsed: !isConfigSectionExpanded(handler.id, 'timing') }"></i>
                                </div>
                                <div class="config-section-body"
                                  v-show="isConfigSectionExpanded(handler.id, 'timing')">
                                  <div class="config-field">
                                    <label class="checkbox-label">
              ...
              <div class="rules-editor-container">
                <div class="rules-editor-toolbar">
                  ...
                  <button @click="resetRulesToDefaults" type="button" class="btn btn-default btn-sm">
                    <i class="ph-duotone ph-arrow-clockwise"></i> Reset ({{ enabledRulesCount }}/{{ totalRulesCount }}
                    enabled)
                  </button>
                </div>
                <div class="rules-list">
                  <div v-if="!settings.rules?.length" class="empty-rules-state">
                  ...
                  <div v-for="(rule, index) in settings.rules || []" :key="rule.id || index" class="rule-card"
                    :class="{ expanded: expandedRules[rule.id || index] }">
                    <div class="rule-card-header" @click="toggleRuleCard(rule.id || index)">
                      <div class="rule-info">
                        <div class="rule-toggle">
                          <input type="checkbox" class="rule-enabled-checkbox checkbox" v-model="rule.enabled"
                            @click.stop />
                          <h4 class="rule-name" :title="rule.name">{{ rule.name }}</h4>
                        </div>
                        <p class="rule-description">
                          {{ rule.examples?.length || 0 }} {{ (rule.examples?.length || 0) === 1 ? 'example' :
                          'examples' }}
                      ...
                      <div class="rule-controls">
                        <i class="ph-duotone ph-arrow-up rule-move-btn" @click.stop="moveRule(index, -1)"
                          :class="{ disabled: index === 0 }" title="Move up"></i>
                        <i class="ph-duotone ph-arrow-down rule-move-btn" @click.stop="moveRule(index, 1)"
                          :class="{ disabled: index === (settings.rules?.length || 0) - 1 }" title="Move down"></i>
                        <button type="button" class="btn btn-icn-only btn-sm" @click.stop="deleteRule(index)"
                          title="Delete rule">
                      ...
                      <div class="rule-form-group">
                        ...
                        <div class="conditions-list">
                          <label class="condition-checkbox"
                            :class="{ active: rule.if && rule.if.includes('selection') }">
                            <input type="checkbox" :checked="rule.if && rule.if.includes('selection')"
                              @change="updateRuleCondition(rule, 'selection', $event.target.checked)" />
                            <i class="ph-duotone ph-selection"></i>
                            Selection
                          </label>
                          <label class="condition-checkbox"
                            :class="{ active: rule.if && rule.if.includes('context') }">
                            <input type="checkbox" :checked="rule.if && rule.if.includes('context')"
                              @change="updateRuleCondition(rule, 'context', $event.target.checked)" />
                            <i class="ph-duotone ph-file-text"></i>
                            Document
                          </label>
                          <label class="condition-checkbox"
                            :class="{ active: rule.if && rule.if.includes('writing_style') }">
                            <input type="checkbox" :checked="rule.if && rule.if.includes('writing_style')"
                          ...
                          <div class="example-controls">
                            <div class="example-inputs">
                              <div class="example-column">
                                <label>From</label>
                                <textarea class="form-control example-textarea" placeholder="Input text..."
                                  v-model="example.from" rows="4"></textarea>
                              </div>
                              <div class="example-column">
                                <label>To</label>
                                <textarea class="form-control example-textarea" placeholder="Expected output..."
                                  v-model="example.to" rows="4"></textarea>
                              </div>
                            </div>
                            <button type="button" class="btn btn-icn-only btn-negative btn-sm example-delete-btn"
                              @click="deleteExample(index, eIndex)" title="Delete example">
                              <i class="ph-duotone ph-trash"></i>
                            </button>
                          </div>
                        </div>
          ...
          <div v-if="currentSection.id === 'data'" class="data-management-section">
            ...
            <div class="section-header-small">
              ...
              <div class="header-actions">
                <button type="button" @click="clearAllPluginData" class="btn btn-negative btn-sm"
                  :disabled="isClearingAll || !pluginDataInfo.length" title="Clear all data from all plugins">
                  <i class="ph-duotone ph-trash"></i>
                  Clear All Data
                </button>
                <button type="button" @click="refreshDataManagement" class="btn btn-sm refresh-btn"
                ...
                <div class="plugin-data-item" :class="{ active: plugin.isActive }">
                    ...
                    <div class="plugin-data-info-content">
                      <div>
                        ...
                        <div class="plugin-data-details-summary">
                          <span class="data-size"><i class="ph-duotone ph-hard-drive"></i> {{
                            formatBytes(plugin.dataSize) }}</span>
                        </div>
                      </div>
                      <div class="plugin-data-actions">
                        <button type="button" @click="togglePluginDetails(plugin.name)"
                          class="btn btn-sm plugin-details-btn" title="View Details">
                          <i class="ph-duotone ph-list-bullets"></i>
                        </button>
                        <button v-if="!plugin.isActive" type="button" @click="clearPluginData(plugin.name)"
                          class="btn btn-negative btn-sm" title="Clear All">
                          <i class="ph-duotone ph-trash"></i>
                        </button>
                      </div>
                    </div>
                  ...
                  <div class="plugin-data-details" v-if="expandedDataPlugins[plugin.name]">
                    <div class="plugin-data-details-header">
                      <h6>Data Items</h6>
                      <span class="item-count">{{ pluginDataItems[plugin.name]?.length || 0 }}
                        items</span>
                    </div>
                    ...
                    <div v-else class="plugin-data-items">
                      <div v-for="item in pluginDataItems[plugin.name]" :key="item.id"
                        ...
                        <div class="item-actions">
                          <button type="button" @click="deletePluginDataItem(plugin.name, item.id, item.name)"
                            class="btn btn-xs btn-negative delete-item-btn" title="Delete">
                            <i class="ph-duotone ph-trash"></i>
                          </button>
                        </div>
                      </div>
        ...
        <div v-if="currentSectionId !== 'permissions'" class="form-actions">
          ...
          <button @click="cancelChanges" type="button" class="btn btn-default">
            <i class="ph-duotone ph-x-circle"></i>
            Cancel
          </button>
          <button @click="resetSection" type="button" class="btn btn-negative">
      ...
      <div v-if="currentSectionId === 'permissions'" class="settings-section">
        ...
        <div class="form-group" :class="{ 'permission-granted': permissions?.accessibility?.granted }">
          ...
          <div class="permission-actions">
            <button type="button" class="btn btn-sm btn-primary" @click="openAccessibilitySettings">
        ...
        <div class="form-group" :class="{ 'permission-granted': permissions?.microphone?.granted }">
          ...
          <div class="permission-actions">
            <button type="button" class="btn btn-sm btn-primary" @click="openMicrophoneSettings">
              <i class="ph-duotone ph-gear"></i>
              Open Settings
            </button>
          </div>
        ...
        <div class="form-group">
          ...
          <div class="permission-actions">
            <button type="button" class="btn btn-default" @click="refreshAllPermissions" :disabled="isRefreshingAll">
              <i v-if="isRefreshingAll" class="ph-duotone ph-circle-notch spinning"></i>
              <i v-else class="ph-duotone ph-arrows-clockwise"></i>
              {{ isRefreshingAll ? 'Refreshing...' : 'Refresh All' }}
            </button>
            <button type="button" class="btn btn-default" @click="openSystemPreferences">
              <i class="ph-duotone ph-gear-six"></i>
        ...
        <div class="form-group info-group">
          ...
          <div class="field-description">
            ...
            <p>If features aren't working after granting permissions, try refreshing the status or restarting the app.
            </p>
    ...
    <main class="settings-content about-content" v-if="currentSectionId === 'about'">
      <div class="about-container">
        <div class="about-header">
          ...
          <a v-if="packageInfo?.repository?.url" href="#" @click.prevent="openExternalLink(getReleasesUrl())"
            class="about-app-version">Version {{ appVersion }}</a>
          ...
          <a v-if="packageInfo?.author" href="#" @click="openAuthorLink" class="about-app-author">By {{
            packageInfo.author }}</a>
          <span v-else class="about-app-author">By Explosion-Scratch</span>
        </div>

        <div class="about-actions">
          <button type="button" @click="importAllSettings" class="btn btn-primary about-action-btn">
            <i class="ph-duotone ph-download-simple"></i>
            Import Settings
          </button>
          <button type="button" @click="exportAllSettings" class="btn btn-primary about-action-btn">
        ...
        <div class="about-footer">
          ...
          <div v-if="packageInfo" class="about-footer-details">
            ...
            <p v-if="packageInfo.homepage" class="about-footer-text">
              <a href="#" @click.prevent="openExternalLink(packageInfo.homepage)"
                class="about-footer-link">Homepage</a>
              <span v-if="packageInfo.bugs?.url"> • </span>
              <a v-if="packageInfo.bugs?.url" href="#" @click.prevent="openExternalLink(packageInfo.bugs.url)"
                class="about-footer-link">Report Issues</a>
              <span v-if="packageInfo.repository?.url"> • </span>
              <a v-if="packageInfo.repository?.url" href="#" @click.prevent="openExternalLink(getRepoUrl())"
                class="about-footer-link">Repository</a>
            </p>
          </div>
        </div>
      </div>
  ...
  <div class="status-message" :class="[status.type, { show: status.visible }]">
    {{ status.message }}
  </div>
